...
> **Superseded. History only. Do not build or deploy from this checkout.**
> The live ratowskyrealestate.com source is `Auto-Authority/client-justin-ratowsky`
> on `main`, hosted on the `ratowsky-real-estate` Cloudflare Worker (not Vercel)
> since 2026-09-05. Nothing in this repository is deployable: it has no Cloudflare
> configuration, its dependencies and content are frozen at June 2026, and the
> setup notes below describe that old state. Open the canonical repository for
> any change.
...
- `GET /api/listings?community=<slug>` — Trestle listings filtered to that page's coverage. A parent (`huntington-harbour`) returns every island plus the Mainland. Pages of candidates are followed through `@odata.nextLink` until `top` matches are found; the coarse page size scales with `top`, at most 600 candidates are examined per request, and the response carries `truncated: true` if that cap stopped the crawl first. Default sort is newest first; add `sort=price` or `sort=price-asc`. Every listing carries `communityMatchedBy` (`subdivision`, `street`, or `polygon`); add `polygon=verified` to drop pin-only matches on rings that have not yet been reviewed (`precision: "verified"`).
